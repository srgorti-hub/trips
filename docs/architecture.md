...
Single HTML file. Reads `trip-data.json` for day structure and estimated stats.
...
│
├── <body>
│   ├── Header (back link, title, trip name, instructions)
│   ├── Day card grid
│   │   ├── Results summary table (appears after processing)
│   │   └── Per-day cards (drop zone + comparison table + mini chart)
│   └── Download bar (updated trip-data.json + elevation zip)
│
└── <script>
    ├── Load trip-data.json (get day count, segment names, estimated stats)
    ├── GPX parsing engine (XML → track points)
    ├── Haversine distance calculator
    ├── Stats calculator (distance, ascent, descent, max elevation)
    ├── Elevation profile generator (sampled at 0.5km intervals)
    ├── Comparison table renderer (estimated vs. GPX with diff %)
    ├── Mini SVG chart renderer (per-day elevation profile)
    ├── Export: updated trip-data.json (GPX stats replace estimated)
    ├── Export: elevation data zip (JSZip)
    └── Init
```
...
```
/TravelPlanner/                ← Project root
├── trip-template.md           ← Upload to Claude.ai
├── docs/                      ← Project documentation
│
├── west-highland-way/         ← Trip folder
│   ├── index.html             ← Travel Guide App
│   ├── photos.html            ← Photo Manager tool
│   ├── gpx-tool.html          ← GPX Data Processor tool
│   ├── trip-data.json         ← Generated by Claude.ai
│   ├── README.md              ← Trip-specific instructions
│   ├── photos/
...
│
├── patagonia/                 ← Another trip folder (same structure)
│   ├── index.html
│   ├── photos.html
│   ├── gpx-tool.html
│   ├── trip-data.json
│   └── ...
│
└── [new-trip]/                ← Copy index.html, photos.html, gpx-tool.html into each new trip
```

All three HTML files (`index.html`, `photos.html`, `gpx-tool.html`) are trip-agnostic — they `fetch('trip-data.json')` from their own directory. To set up a new trip, copy all three into the trip folder alongside the generated data files. Each trip's `index.html` footer links to `photos.html` and `gpx-tool.html`.
